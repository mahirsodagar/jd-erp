"""Payment public endpoints (SmartGateway + Razorpay) + a small read API.
...
* `RazorpayWebhookView` — server-to-server, authenticated by the HMAC
                         signature over the body.

The pay and return views serve both gateways; `PaymentRequest.gateway`
decides which client they talk to.

All of these declare `authentication_classes = []` so DRF's SessionAuth
...
class PayRedirectView(APIView):
    ...
    def get(self, request, token):
        ...
        if not gateway_usable(payment_request.gateway, payment_request.account):
            logger.error(
                "payments: pay link opened for request %s but %s is not "
                "usable for account %r (missing: %s).",
                payment_request.pk, payment_request.gateway,
                payment_request.account,
                ", ".join(gateway_missing_settings(
                    payment_request.gateway, payment_request.account,
                )) or "the master switch",
            )
...
class PayReturnView(APIView):
    ...
    def _handle(self, request, token):
        ...
        is_razorpay = payment_request.gateway == routing.GATEWAY_RAZORPAY
        if is_razorpay:
            # Razorpay's callback: razorpay_payment_link_status is "paid"
            # on success; mapped onto our vocabulary for the fallback below.
            signed_ok = razorpay.verify_callback_signature(params)
            signed_status = razorpay.LINK_STATUS_MAP.get(
                str(params.get("razorpay_payment_link_status") or "").lower(), "",
            )
            has_signature = bool(params.get("razorpay_signature"))
            # The payer may have paid an older link still open in a tab;
            # settle the one the callback names, not merely the newest.
            named = payment_request.orders.filter(
                order_id=params.get("razorpay_payment_link_reference_id") or "",
            ).first()
            if named is not None:
                order = named
        else:
            signed_ok = (
                verify_return_signature(params, payment_request.account)
                if params else False
            )
            signed_status = str(params.get("status") or "").upper()
            has_signature = bool(params.get("signature"))
        if has_signature and not signed_ok:
            ...
            logger.warning(
                ...
                "RAZORPAY_KEY_SECRET" if is_razorpay
                else "SMARTGATEWAY_RESPONSE_KEY",
            )
...
class SmartGatewayStatusView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            # One row per settlement account, so "why can't this JDSD
            # student pay?" is answerable without shelling into the box.
            "accounts": [
                {
                    "key": key,
                    "label": meta["label"],
                    ...
                    "missing_settings": missing_settings(key),
                    "routes_by_reference_id": bool(
                        merchant_config(key).gateway_reference_id,
                    ),
                }
                for key, meta in routing.ACCOUNTS.items()
            ],
            "routes": routing.routes(),
            "razorpay": {
                "enabled": razorpay.is_enabled(),
                "misconfigured": bool(
                    getattr(settings, "RAZORPAY_ENABLED", False)
                    and razorpay.missing_settings(),
                ),
                "missing_settings": razorpay.missing_settings(),
                "mode": (
                    ("test" if razorpay.is_test_mode() else "live")
                    if razorpay.is_enabled() else None
                ),
                "webhook_configured": bool(
...
class RazorpayWebhookView(APIView):
    """`POST /api/public/razorpay/webhook/` — Razorpay's webhook.

    Authenticated by `X-Razorpay-Signature`, an HMAC of the raw body with
    the dashboard's webhook secret. Same answering rule as SmartGateway's:
    200 for anything durably recorded, 4xx only for a bad signature or an
    unparseable body.
    """
    ...
    def post(self, request):
        ...
        if not razorpay.verify_webhook_signature(
            ...
            logger.warning(
                "payments: rejected Razorpay webhook with a bad/missing "
                "signature (or RAZORPAY_WEBHOOK_SECRET unset).",
            )
            return Response(
        ...
        event_id = (
            ...
            or f"{body.get('event')}:{link.get('id')}:{body.get('created_at')}"
        )
        event = process_razorpay_webhook(event_id=event_id, body=body)
        return Response(
            {"status": event.status, "event_id": event.event_id},
            status=http.HTTP_200_OK,
        )
